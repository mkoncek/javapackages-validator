...
public class JavaExclusiveArchCheckTest {
    ...
    public void testAllowedExclusiveArchArchful() throws IOException {
        var result = new JavaExclusiveArchCheck().check(TestCommon.collectionFrom(Stream.of(EA_ARCHFUL)));
        assertEquals(0, result.size());
    }

    @Test
    public void testAllowedExclusiveArchNoarch() throws IOException {
        var result = new JavaExclusiveArchCheck().check(TestCommon.collectionFrom(Stream.of(EA_NOARCH)));
        assertEquals(0, result.size());
    }

    @Test
    public void testExclusiveArchMissingNoarch() throws IOException {
        var result = new JavaExclusiveArchCheck().check(TestCommon.collectionFrom(Stream.of(EA_ARCHFUL_MISSING)));
        assertEquals(1, result.size());
    }

    @Test
    public void testIllegalExclusiveArchAdditionalNoarch() throws IOException {
        var result = new JavaExclusiveArchCheck().check(TestCommon.collectionFrom(Stream.of(EA_ARCHFUL_NOARCH)));
        assertEquals(1, result.size());
    }
}
